...
from ..models import BaseResponse, Session, SessionSummary
...
async def create_session(session: Session):
    ...
    try:
        ...
        await websocket_manager.broadcast({"type": "session_created", "data": session.dict()})
        ...
        for i, existing_session in enumerate(sessions):
            if existing_session.id == session_id:
                ...
                await websocket_manager.broadcast(
                    {"type": "session_updated", "data": session.dict()}
                )
