...
class LiveAttackTester:
        ...
        for payload in sql_payloads:
            try:
                ...
                if found_indicators:
                    ...
                    logger.info(
                        f"✅ SQL injection found with payload '{payload}': {found_indicators}"
                    )
                else:
        ...
        for payload in cmd_payloads:
            try:
                ...
                if found_indicators:
                    ...
                    vulnerabilities.append(vulnerability)
                    logger.info(
                        f"✅ Command injection found with payload '{payload}': {found_indicators}"
                    )
                else:
    ...
    def test_fuzzing(self, target_name: str, target_url: str) -> Dict:
        ...
        try:
            ...
            # Analyze results
            successful_attacks = [
                r for r in results if hasattr(r, "is_vulnerable") and r.is_vulnerable
            ]
            ...
            logger.info(
                f"✅ Fuzzed {target_name}: {len(successful_attacks)}/{len(results)} successful attacks"
            )
    ...
    def print_results(self, results: Dict):
        ...
        logger.info(
            f"⚔️ Attack Success Rate: {successful_attacks}/{total_targets} ({successful_attacks / total_targets * 100:.1f}%)"
        )
        ...
        for target, metrics in results["performance_metrics"].items():
            if "crawling" in metrics:
                ...
                logger.info(
                    f"   {target} - Crawling: {crawl.get('pages_discovered', 0)} pages in {crawl.get('crawl_time', 0):.2f}s"
                )

            if "fuzzing" in metrics:
                fuzz = metrics["fuzzing"]
                logger.info(
                    f"   {target} - Fuzzing: {fuzz.get('successful_attacks', 0)}/{fuzz.get('total_tests', 0)} successful"
                )
