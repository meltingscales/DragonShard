...
class FullWorkflowTester:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "docker-compose",
                    "-f",
                    os.path.join(project_root, "docker-compose.test.yml"),
                    "ps",
                ],
                capture_output=True,
                text=True,
    ...
    def step3_fuzzing(self, target_url: str, endpoints: List[Dict]) -> Dict:
        ...
        try:
                ...
                for endpoint in config["endpoints"]:
                    ...
                    for ep in matching_endpoints:
                        ...
                        results = fuzzer.fuzz_url(
                            url=test_url, method="POST", payload_types=[vuln_type]
                        )
            ...
            for result in fuzzing_results:
                logger.info(
                    f"   {result['vulnerability_type']} on {result['endpoint']}: {result['successful_attacks']} attacks"
                )
    ...
    def step5_execution(self, attack_chains: List[Dict]) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"✅ Execution completed: {successful_executions}/{len(attack_chains)} successful"
            )
    ...
    def step6_post_exploitation(self, target_url: str) -> Dict:
        ...
        try:
            ...
            logger.info("✅ Post-Exploitation completed: Reverse shell listener created")
    ...
    def run_full_workflow(self, target_name: str, target_url: str) -> Dict:
        ...
        logger.info("🚀 Running Full DragonShard Workflow")
    ...
    def print_workflow_results(self, results: Dict):
        ...
        logger.info(
            f"   🔍 Reconnaissance: {steps.get('reconnaissance', {}).get('open_ports', 0)} open ports"
        )
        logger.info(
            f"   🕷️ API Inference: {steps.get('api_inference', {}).get('endpoints_discovered', 0)} endpoints"
        )
        logger.info(
            f"   🧬 Fuzzing: {steps.get('fuzzing', {}).get('total_vulnerabilities_found', 0)} vulnerabilities"
        )
        logger.info(
            f"   🧠 Planning: {steps.get('planning', {}).get('attack_chains_generated', 0)} attack chains"
        )
        logger.info(
            f"   ⚔️ Execution: {steps.get('execution', {}).get('successful_executions', 0)} successful"
        )
        logger.info(
            f"   🐚 Post-Exploitation: {'✅' if steps.get('post_exploitation', {}).get('reverse_shell_created') else '❌'}"
        )
        ...
        logger.info(
            f"   Reverse Shell Created: {'✅' if summary.get('reverse_shell_created') else '❌'}"
        )
...
def main():
    ...
    # Test targets
    targets = {"vuln-php": "http://localhost:8082", "vuln-node": "http://localhost:8083"}
