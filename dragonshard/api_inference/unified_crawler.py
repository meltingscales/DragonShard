...
def smart_crawl(
    start_url: str, max_depth: int = 2, max_pages: int = 50, force_js: bool = False, **kwargs
) -> Set[str]:
    """
...
def crawl_with_js(
    ...
    **kwargs,
    ...
    """
    with JSCrawler(
        max_depth=max_depth, max_pages=max_pages, wait_time=wait_time, headless=headless, **kwargs
    ) as crawler:
        return crawler.crawl(start_url)
...
def compare_crawlers(start_url: str, max_depth: int = 1, max_pages: int = 10) -> Dict[str, Any]:
    ...
    results["fast"] = {"urls": fast_results, "count": len(fast_results), "time": fast_time}
    ...
    results["js"] = {"urls": js_results, "count": len(js_results), "time": js_time}
