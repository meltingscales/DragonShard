...
def main():
    ...
    if not (Path(__file__).parent / "api").exists():
        print(
            "❌ Error: API directory not found. Please run this script from the visualizer directory."
        )
        sys.exit(1)

    if not (Path(__file__).parent / "frontend").exists():
        print(
            "❌ Error: Frontend directory not found. Please run this script from the visualizer directory."
        )
        sys.exit(1)
