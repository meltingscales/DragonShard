...
class GeneticTreeIntegration:
    ...
    def setup_ui(self):
        ...
        # Payload type
        ttk.Label(control_frame, text="Payload Type:").grid(
        ...
        self.start_button = ttk.Button(
            control_frame, text="Start Evolution", command=self.start_evolution
        )
        self.start_button.grid(row=0, column=8, padx=(0, 5))

        self.stop_button = ttk.Button(
            control_frame, text="Stop Evolution", command=self.stop_evolution, state=tk.DISABLED
        )
    ...
    def setup_details_panel(self, parent):
        ...
        ttk.Label(gen_frame, textvariable=self.gen_var, font=("Arial", 12, "bold")).pack(
        ...
        ttk.Label(
            fitness_frame, textvariable=self.best_fitness_var, font=("Arial", 12, "bold")
        ...
        ttk.Label(vuln_frame, textvariable=self.vuln_count_var, font=("Arial", 12, "bold")).pack(
            side=tk.LEFT, padx=(5, 0)
        )
    ...
    def run_evolution(self):
        ...
        try:
            ...
            self.log_message(
                f"Initialized population with {len(self.mutator.population)} individuals"
            )
    ...
    def calculate_fitness(self, payload: GeneticPayload) -> float:
        ...
        if payload.payload_type == PayloadType.SQL_INJECTION:
            if any(
                keyword in payload.payload.lower() for keyword in ["union", "select", "or", "and"]
            ):
                fitness += 0.3
    ...
    def get_base_payloads(self, payload_type: PayloadType) -> List[str]:
        ...
        if payload_type == PayloadType.SQL_INJECTION:
            ...
            return ["; ls -la", "| whoami", "&& cat /etc/passwd", "; id", "| uname -a"]
    ...
    def update_loop(self):
        ...
        if self.running:
            ...
            if self.tree_visualizer and self.tree_visualizer.nodes:
                vuln_count = sum(
                    1 for node in self.tree_visualizer.nodes.values() if node.vulnerability_detected
                )
