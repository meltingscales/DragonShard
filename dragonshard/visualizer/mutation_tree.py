...
class MutationTreeVisualizer:
    ...
    def setup_ui(self):
        ...
        # Action buttons
        ttk.Button(control_frame, text="Clear Tree", command=self.clear_tree).grid(
            row=0, column=8, padx=(0, 5)
        )
        ttk.Button(control_frame, text="Export Tree", command=self.export_tree).grid(
            row=0, column=9, padx=(0, 5)
        )
        ttk.Button(control_frame, text="Find Path", command=self.find_best_path).grid(
            row=0, column=10, padx=(0, 5)
        )
    ...
    def setup_tree_view(self):
        ...
        tree_scroll_x = ttk.Scrollbar(
            tree_frame, orient=tk.HORIZONTAL, command=self.tree_view.xview
        )
    ...
    def update_tree_view(self):
        ...
        for node_id, node in self.nodes.items():
            ...
            payload_display = (
                node.payload.payload[:30] + "..."
                if len(node.payload.payload) > 30
                else node.payload.payload
            )
    ...
    def update_visualization(self):
        ...
        self.ax.set_title(
            f"Mutation Tree (Generation {self.current_generation}) - {len(display_nodes)}/{len(self.nodes)} nodes"
        )
        self.canvas.draw()
...
"""
    ...
    def find_best_path(self):
        ...
        vulnerable_nodes = [
            node_id for node_id, node in self.nodes.items() if node.vulnerability_detected
        ]
        ...
        best_vulnerable = max(
            vulnerable_nodes, key=lambda node_id: self.nodes[node_id].fitness_score
        )
