...
class GeneticAlgorithmVisualizer:
    ...
    def setup_ui(self):
        ...
        ttk.Label(info_frame, textvariable=self.generation_var).grid(
        ...
        ttk.Label(info_frame, textvariable=self.population_var).grid(
        ...
        ttk.Label(info_frame, textvariable=self.best_fitness_var).grid(
        ...
        payloads_scrollbar = ttk.Scrollbar(
            payloads_frame, orient=tk.VERTICAL, command=self.payloads_text.yview
        )
        ...
        try:
            for generation in range(self.mutator.max_generations):
                ...
                generation_data = {
                    ...
                    "avg_fitness": sum(p.fitness for p in best_payloads) / len(best_payloads)
                    if best_payloads
                    else 0.0,
    ...
    def update_visualization(self, data: Dict[str, Any]):
        ...
        for i, payload in enumerate(data["best_payloads"], 1):
            self.payloads_text.insert(
                tk.END, f"{i}. {payload.payload[:50]}... (fitness: {payload.fitness:.3f})\n"
            )
    ...
    def update_mutation_chart(self, data: Dict[str, Any]):
        ...
        bars = self.mutation_ax.bar(
            mutation_types, mutation_counts, color=["red", "blue", "green", "orange"]
        )
        self.mutation_ax.set_title("Mutation Types")
        self.mutation_ax.set_ylabel("Mutation Count")
