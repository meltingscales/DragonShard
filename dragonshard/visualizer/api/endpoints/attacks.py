...
from ..models import (
    AttackChain, AttackStep, AttackSummary, AttackStatus,
    BaseResponse, WebSocketMessage
)
...
async def get_attack_summary():
    ...
    try:
        ...
        average_duration = sum(a.duration for a in completed_with_duration) / len(completed_with_duration) if completed_with_duration else 0
        ...
        for attack in attack_chains:
            if attack.id == attack_id:
                ...
                if status == AttackStatus.RUNNING and not attack.start_time:
                    attack.start_time = datetime.now()
                elif status in [AttackStatus.COMPLETED, AttackStatus.FAILED, AttackStatus.CANCELLED]:
            ...
            if attack.id == attack_id:
                for i, existing_step in enumerate(attack.steps):
                    if existing_step.id == step_id:
                        ...
                        completed_steps = len([s for s in attack.steps if s.status == AttackStatus.COMPLETED])
                        attack.completed_steps = completed_steps
                        attack.success_rate = (completed_steps / attack.total_steps * 100) if attack.total_steps > 0 else 0
