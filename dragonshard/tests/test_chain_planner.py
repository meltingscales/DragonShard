...
from dragonshard.planner.chain_planner import (
    ...
    AttackImpact, AttackStep, AttackChain
)
...
from dragonshard.planner.vulnerability_prioritization import (
    VulnerabilityPrioritizer, VulnerabilityScore, RiskLevel
)
...
class TestVulnerabilityPrioritizer(unittest.TestCase):
    ...
    def test_prioritize_vulnerabilities(self):
        ...
        for i in range(len(scored_vulns) - 1):
            self.assertGreaterEqual(
                scored_vulns[i].risk_score, 
                scored_vulns[i + 1].risk_score
            )
    ...
    def test_get_critical_vulnerabilities(self):
        ...
        for vuln in critical_vulns:
            ...
            self.assertTrue(
                vuln.vulnerability_type == AttackType.COMMAND_INJECTION or 
                vuln.impact == AttackImpact.CRITICAL
            )

    def test_get_high_risk_vulnerabilities(self):
        """Test getting high-risk vulnerabilities."""
        high_risk_vulns = self.prioritizer.get_high_risk_vulnerabilities(self.sample_vulnerabilities)
        ...
        for vuln in high_risk_vulns:
            ...
            self.assertTrue(
                vuln.impact in [AttackImpact.HIGH, AttackImpact.CRITICAL]
            )
