...
from dragonshard.executor.executor import AttackExecutor, ExecutionStatus, ExecutionResult
from dragonshard.executor.session_manager import SessionManager, SessionState, AuthMethod, AuthCredentials
...
class TestExecutorStressScenarios(unittest.TestCase):
    ...
    def test_session_management_integration(self):
        ...
        authenticated_sessions = [s for s in sessions if s.get('state') == SessionState.AUTHENTICATED.value]
    ...
    def test_state_graph_integration(self):
        ...
        service1_id = self.state_graph.add_service(host_id, 3389, ServiceType.HTTPS, "Microsoft RDP")
        service2_id = self.state_graph.add_service(host_id, 445, ServiceType.HTTPS, "Microsoft SMB")
        service3_id = self.state_graph.add_service(host_id, 443, ServiceType.HTTPS, "Microsoft Exchange")
        service4_id = self.state_graph.add_service(host_id, 9443, ServiceType.HTTPS, "VMware vCenter")

        # Add vulnerabilities
        self.state_graph.add_vulnerability(
            service1_id, "CVE-2019-0708", VulnerabilityLevel.CRITICAL,
            "BlueKeep", "RDP vulnerability"
        )
        self.state_graph.add_vulnerability(
            service2_id, "CVE-2021-34527", VulnerabilityLevel.CRITICAL,
            "PrintNightmare", "SMB vulnerability"
        )
        self.state_graph.add_vulnerability(
            service3_id, "CVE-2021-26855", VulnerabilityLevel.CRITICAL,
            "ProxyLogon", "Exchange vulnerability"
        )
        self.state_graph.add_vulnerability(
            service4_id, "CVE-2021-21972", VulnerabilityLevel.CRITICAL,
            "vCenter SSRF", "vCenter vulnerability"
        )
    ...
    def test_export_capabilities(self):
        ...
        from dragonshard.executor.executor import ExecutionStep, ExecutionStatus, ExecutionResult
