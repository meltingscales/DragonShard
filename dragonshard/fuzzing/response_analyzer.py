...
class ResponseAnalyzer:
    ...
    def _load_vulnerability_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "xxe": [r"xml.*error", r"entity.*not found", r"xml.*parse.*error", r"external.*entity"],
        }
    ...
    def _classify_response(
        self, status_code: int, vulnerability_indicators: List[str]
    ) -> ResponseType:
    ...
    def _calculate_anomaly_score(
        self, status_code: int, response_time: float, content_length: int
    ) -> float:
        ...
        if self.response_history:
            avg_time = sum(r.response_time for r in self.response_history) / len(
        ...
        if self.response_history:
            avg_length = sum(r.content_length for r in self.response_history) / len(
                self.response_history
            )
            if (
                abs(content_length - avg_length) / avg_length
                > self.anomaly_thresholds["content_length_deviation"]
            ):
                score += 0.5
    ...
    def _calculate_baseline_deviation(
        ...
        if baseline.content_length > 0:
            length_deviation = (
                abs(content_length - baseline.content_length) / baseline.content_length
            )
    ...
    def compare_responses(
        self, baseline: ResponseAnalysis, test: ResponseAnalysis
    ...
    def _calculate_similarity(self, baseline: ResponseAnalysis, test: ResponseAnalysis) -> float:
        ...
        if baseline.content_length > 0:
            length_diff = (
                abs(test.content_length - baseline.content_length) / baseline.content_length
            )
            similarities.append(1.0 - min(1.0, length_diff))

        return sum(similarities) / len(similarities)

    def _find_differential_indicators(
        self, baseline: ResponseAnalysis, test: ResponseAnalysis
    ) -> List[str]:
        ...
        if baseline.response_type != test.response_type:
            indicators.append(
                f"response_type_change:{baseline.response_type.value}->{test.response_type.value}"
            )

        return indicators

    def _calculate_reward_score(
        self, baseline: ResponseAnalysis, test: ResponseAnalysis, differential_indicators: List[str]
    ) -> float:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_response_time": sum(r.response_time for r in self.response_history)
            / len(self.response_history),
            "avg_content_length": sum(r.content_length for r in self.response_history)
            / len(self.response_history),
            "vulnerability_rate": len(
                [r for r in self.response_history if r.vulnerability_indicators]
            )
            / len(self.response_history),
            "error_rate": len([r for r in self.response_history if r.status_code >= 400])
            / len(self.response_history),
        }
