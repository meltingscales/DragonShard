...
class AnomalyDetector:
        ...
        for error_type, patterns in self.response_patterns["error_patterns"].items():
            for pattern in patterns:
                if re.search(pattern, response_lower, re.IGNORECASE):
                    ...
                    severity = (
                        "high" if error_type in ["sql_errors", "command_injection"] else "medium"
                    )
    ...
    def _detect_size_anomalies(
        self, response_size: int, baseline_response: Optional[Dict]
    ) -> List[AnomalyResult]:
    ...
    def _detect_status_anomalies(
        self, status_code: int, baseline_response: Optional[Dict]
    ) -> List[AnomalyResult]:
    ...
    def _detect_timing_anomalies(
        self, response_headers: Dict[str, str], baseline_response: Optional[Dict]
    ) -> List[AnomalyResult]:
    ...
    def _detect_content_anomalies(
        self, response_text: str, baseline_response: Optional[Dict]
    ) -> List[AnomalyResult]:
