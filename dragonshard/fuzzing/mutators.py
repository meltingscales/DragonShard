...
class PayloadMutator:
    ...
    def generate_payload_combinations(self, payloads: List[str], 
                                    max_combinations: int = 10) -> List[str]:
        """
        ...
        for _ in range(min(max_combinations, len(payloads) * 2)):
            if random.random() < 0.5:
                ...
                separators = [' ', ';', '|', '&', '&&', '||']
                combo = random.choice(separators).join(random.sample(payloads, random.randint(2, min(3, len(payloads)))))
                combinations.append(combo)
