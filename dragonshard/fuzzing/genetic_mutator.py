...
class GeneticMutator:
    ...
    def _rank_based_selection(self) -> GeneticPayload:
        ...
        for i, individual in enumerate(sorted_population):
            rank = i
            probability = (2 - selection_pressure) / n + 2 * rank * (selection_pressure - 1) / (
                n * (n - 1)
            )
    ...
    def _adapt_mutation_rate(self) -> float:
        ...
        new_rate = (
            ...
            + (self.max_mutation_rate - self.min_mutation_rate) * adaptation_factor
        )

        # Log adaptation for debugging
        logger.debug(
            f"Adaptive mutation: diversity={diversity:.3f}, convergence={convergence:.3f}, "
            f"rate={new_rate:.3f}"
        )
    ...
    def create_multi_objective_fitness(
        self, target_url: str, objectives: List[str] = None, weights: List[float] = None
    ) -> Callable:
    ...
    def _calculate_vulnerability_score(self, payload: GeneticPayload) -> float:
        ...
        if payload.payload_type == PayloadType.SQL_INJECTION:
            ...
            score += sum(
                0.1 for indicator in sql_indicators if indicator in payload.payload.upper()
            )

        elif payload.payload_type == PayloadType.XSS:
            xss_indicators = ["<script>", "javascript:", "alert(", "onload=", "onerror="]
            score += sum(
                0.15 for indicator in xss_indicators if indicator.lower() in payload.payload.lower()
            )
    ...
    def _calculate_evasion_score(self, payload: GeneticPayload) -> float:
        ...
        if (
            payload.payload != payload.payload.lower()
            and payload.payload != payload.payload.upper()
        ):
            score += 0.2
