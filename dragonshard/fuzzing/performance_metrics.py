...
class PerformanceMetrics:
    ...
    def _calculate_diversity(self, population: List[GeneticPayload]) -> float:
        ...
        for i in range(len(population)):
            for j in range(i + 1, len(population)):
                similarity = self._payload_similarity(
                    population[i].payload, population[j].payload
                )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_generation_time": statistics.mean(self.generation_times) if self.generation_times else 0.0,
            "total_requests": self.total_requests,
            "avg_request_time": self.avg_request_time,
            "current_best_fitness": self.best_fitness_history[-1] if self.best_fitness_history else 0.0,
            "current_avg_fitness": self.avg_fitness_history[-1] if self.avg_fitness_history else 0.0,
            "fitness_improvement_rate": self.fitness_improvement_rate,
            "current_diversity": self.population_diversity[-1] if self.population_diversity else 1.0,
