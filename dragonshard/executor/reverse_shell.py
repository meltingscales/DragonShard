...
class ReverseShellHandler:
    ...
    def __init__(self, base_port: int = 4444, max_ports: int = 100):
        ...
        logger.info(f"ReverseShellHandler initialized with base_port={base_port}, max_ports={max_ports}")
    ...
    def _handle_connection(self, connection_id: str, client_socket: socket.socket) -> None:
        ...
        try:
            ...
            self._trigger_callbacks('connection', connection_id, {
    ...
    def _read_from_shell(self, connection_id: str, client_socket: socket.socket, output_queue: queue.Queue) -> None:
        ...
        try:
            while connection.status == ShellStatus.CONNECTED:
                try:
                    ...
                    connection.console_history.append(f"[{datetime.now().isoformat()}] {output_text}")
                    
                    # Trim history if it gets too long
                    if len(connection.console_history) > connection.max_history_size:
                        connection.console_history = connection.console_history[-connection.max_history_size:]
    ...
    def _write_to_shell(self, connection_id: str, client_socket: socket.socket, input_queue: queue.Queue) -> None:
        ...
        try:
            while connection.status == ShellStatus.CONNECTED:
                try:
                    ...
                    connection.console_history.append(f"[{datetime.now().isoformat()}] > {input_text}")
